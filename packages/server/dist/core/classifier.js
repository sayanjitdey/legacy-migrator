...
function hasExtractableStateDeclaration(cls) {
    const ctor = cls.getConstructors()[0];
    if (ctor) {
        const body = ctor.getBody();
        if (body && body.getText().includes("this.state"))
            return true;
    }
    const stateProperty = cls.getProperty("state");
    return !!stateProperty?.getInitializer();
}
function hasUnsupportedSetStatePattern(cls) {
    ...
    for (const call of setStateCalls) {
        ...
        if (!arg)
            continue;
        const isFunctionalUpdater = arg.getKind() === ts_morph_1.SyntaxKind.ArrowFunction ||
            arg.getKind() === ts_morph_1.SyntaxKind.FunctionExpression;
        if (isFunctionalUpdater)
            return true;
        const objectLiteral = arg.asKind(ts_morph_1.SyntaxKind.ObjectLiteralExpression);
        if (objectLiteral && objectLiteral.getProperties().length > 1)
            return true;
    }
    return false;
}
function isReactClassComponent(cls) {
    ...
    return /React\.Component|React\.PureComponent|^Component$|^PureComponent$/.test(text);
}
function getLifecycleMethods(cls) {
    return cls
        .getMethods()
        .filter((m) => KNOWN_LIFECYCLE_METHODS.includes(m.getName()));
}
function detectRefs(cls) {
 ...
 */
function detectHOCWrapping(sourceFile, className) {
    ...
    if (defaultExportSymbol) {
        ...
        if (decl && decl.getText().includes(`(${className})`))
            return true;
    }
    return false;
}
 ...
 */
function detectLikelyImperativeExposure(cls) {
...
function assessLifecycleComplexity(method) {
    const name = method.getName();
    const body = method.getBody();
    if (!body)
        return { isMechanical: true, reason: `${name} has no body` };
    const statements = body.getDescendantsOfKind(ts_morph_1.SyntaxKind.IfStatement);
...
function classifyFile(sourceFile) {
    ...
    for (const cls of sourceFile.getClasses()) {
        ...
        const hasShouldComponentUpdate = lifecycleMethodDecls.some((m) => m.getName() === "shouldComponentUpdate");
        const hasInstanceMethodsExposedExternally = detectLikelyImperativeExposure(cls);
        const hasState = cls.getText().includes("this.state") ||
            cls.getText().includes("this.setState");
        const reasons = [];
        const lifecycleMethods = lifecycleMethodDecls.map((m) => ({
            name: m.getName(),
        ...
        if (isWrappedByHOC) {
            reasons.push("Component is wrapped by a HOC in this file — migration must preserve wrapper behavior; not safe to auto-migrate in isolation.");
        }
        if (hasInstanceMethodsExposedExternally) {
            reasons.push("Component appears to expose instance methods imperatively to parents (ref-based API) — hooks equivalent (useImperativeHandle) requires manual review.");
        }
        ...
        if (hasShouldComponentUpdate) {
            reasons.push("shouldComponentUpdate present — translating to React.memo/useMemo comparison logic requires judgment.");
        }
        if (!hasExtractableStateDeclaration(cls) && hasState) {
            reasons.push("State is referenced but not declared in a constructor or as a simple class field — cannot reliably extract initial state shape for useState.");
        }
        if (hasUnsupportedSetStatePattern(cls)) {
            reasons.push("Contains multi-key or functional-updater setState calls — the deterministic codemod only rewrites single-key setState calls safely; this needs judgment to translate correctly.");
        }
        ...
        const stateDeclarationUnextractable = !hasExtractableStateDeclaration(cls) && hasState;
        const unsupportedSetState = hasUnsupportedSetStatePattern(cls);
        if (!allLifecycleMechanical ||
            hasShouldComponentUpdate ||
        ...
        reasons.push("All lifecycle methods are simple and map directly to known hook patterns.");
        reports.push({
            fileName: sourceFile.getBaseName(),
            className,
            tier: "MECHANICAL",
