...
app.post("/api/migrate-repo", async (req, res) => {
    ...
    let repoPath;
    try {
        repoPath = await (0, git_service_1.cloneRepo)(repoUrl);
    }
    catch (err) {
    ...
    for (const sourceFile of project.getSourceFiles()) {
        const fileReports = (0, classifier_1.classifyFile)(sourceFile);
    ...
    const jobs = await Promise.all(filePaths.map((filePath) => queue_1.migrationQueue.add("migrate-file", { filePath })));
    res.json({
        repoPath,
        classified: reports,
