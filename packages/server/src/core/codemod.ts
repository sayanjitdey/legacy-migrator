import { SourceFile, ClassDeclaration, SyntaxKind, PropertyAccessExpression } from "ts-morph";
...
function extractStateFields(cls: ClassDeclaration): StateField[] {
  const ctor = cls.getConstructors()[0];
  if (!ctor) return [];

  const body = ctor.getBody()?.asKind(SyntaxKind.Block);
  if (!body) return [];
  ...
  for (const stmt of body.getStatements()) {
    ...
    if (left !== "this.state") continue;

    const right = binExpr.getRight().asKind(SyntaxKind.ObjectLiteralExpression);
    if (!right) continue;
 ...
 */
function mergeReactImport(sourceFile: SourceFile, hooksUsed: string[]): void {
 ...
 */
function rewriteThisReferences(
  text: string,
 ...
 */
function rewriteSetStateCalls(text: string, stateFieldNames: string[]): string {
  let result = text;
  for (const name of stateFieldNames) {
    // Matches: this.setState({ count: <expr> })  (single key only)
    const pattern = new RegExp(
      `this\\.setState\\(\\{\\s*${name}\\s*:\\s*([^}]+)\\}\\)`,
      "g"
    );
...
export function generateHooksComponent(cls: ClassDeclaration): string {
  ...
  const otherProperties = cls
    .getProperties()
    .filter((p) => p.getInitializer()?.getKind() === SyntaxKind.ArrowFunction);
  ...
  for (const prop of otherProperties) {
    ...
    if (!arrowFn) continue;
    const params = arrowFn.asKind(SyntaxKind.ArrowFunction)?.getParameters()
      .map((p) => p.getText()).join(", ") ?? "";
    const bodyText = arrowFn.asKind(SyntaxKind.ArrowFunction)?.getBody().getText() ?? "";
  ...
  const renderBody = renderMethod?.getBody()?.asKind(SyntaxKind.Block);
  ...
  let jsxText = returnStmt?.asKind(SyntaxKind.ReturnStatement)?.getExpression()?.getText() ?? "null";
  jsxText = rewriteThisReferences(jsxText, stateFieldNames, methodNames);

  const hooksUsed: string[] = [];
  if (stateFields.length > 0) hooksUsed.push("useState");
  if (effectBlock) hooksUsed.push("useEffect");
  ...
  // statement).
  const hasExportModifier = cls.hasModifier(SyntaxKind.ExportKeyword);
  const hasDefaultModifier = cls.hasModifier(SyntaxKind.DefaultKeyword);
